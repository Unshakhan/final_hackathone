...
export function LoginPage() {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); setError(""); setLoading(true); try { const next = await login(email.trim(), password); navigate(`/${next.role}/dashboard`, { replace: true }); } catch (requestError) { setError(apiErrorMessage(requestError)); } finally { setLoading(false); } };
...
export function RegisterPage() {
  ...
  if (user) return <Navigate to={`/${user.role}/dashboard`} replace />;
  const submit = async (event: FormEvent) => { event.preventDefault(); setError(""); if (form.name.trim().length < 2) return setError("Name must contain at least 2 characters."); if (form.password.length < 8 || !/[a-z]/.test(form.password) || !/[A-Z]/.test(form.password) || !/\d/.test(form.password)) return setError("Password needs 8+ characters with uppercase, lowercase, and a number."); setLoading(true); try { const next = await register(form.name.trim(), form.email.trim(), form.password); navigate(`/${next.role}/dashboard`, { replace: true }); } catch (requestError) { setError(apiErrorMessage(requestError)); } finally { setLoading(false); } };
