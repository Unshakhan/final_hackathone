...
export function CustomerTicketDetailsPage() {
  ...
  return <div className="page-stack"><div className="detail-topline"><Link className="back-link" to="/customer/tickets">← Back to my tickets</Link><ConnectionState status={socketState} /></div><TicketSummary ticket={state.ticket} /><Conversation ticketId={state.ticket._id} basePath="/api/tickets" resolved={state.ticket.status === "Resolved"} /></div>;
}
...
export function AgentTicketDetailsPage() {
  ...
  return <div className="page-stack"><div className="detail-topline"><Link className="back-link" to="/agent/tickets">← Back to assigned tickets</Link><ConnectionState status={socketState} /></div>{notice && <Notice type={notice.type}>{notice.text}</Notice>}<TicketSummary ticket={ticket} agentView />
    <section className="agent-workspace"><article className="panel"><div className="panel-heading"><div><span className="review-label ai">✦ AI Suggestion</span><h2>Suggested triage</h2></div></div>{ticket.aiSuggestion ? <div className="suggestion-box"><div><span>Category</span><strong>{ticket.aiSuggestion.category}</strong></div><div><span>Priority</span><strong>{ticket.aiSuggestion.priority}</strong></div><p>{ticket.aiSuggestion.summary}</p></div> : <p className="muted">No AI suggestion was stored.</p>}</article>
