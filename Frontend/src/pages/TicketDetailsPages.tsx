...
export function AgentTicketDetailsPage() {
  ...
  const saveTriage = async (event: FormEvent) => { event.preventDefault(); if (!triage) return; setReviewing(true); setNotice(null); try { await api.patch(`/api/agent/tickets/${ticket._id}/triage`, triage); setNotice({ type: "success", text: "Human review saved." }); toast.success("AI review saved.", { id: `triage-saved-${ticket._id}` }); await state.load(); } catch (requestError) { const text = apiErrorMessage(requestError); setNotice({ type: "error", text }); toast.error(text, { id: `triage-error-${ticket._id}` }); } finally { setReviewing(false); } };
  const nextStatus = ticket.status === "New" ? "Assigned" : ticket.status === "Assigned" ? "In Progress" : null;
  const advance = async () => { if (!nextStatus) return; setStatusLoading(true); setNotice(null); try { await api.patch(`/api/agent/tickets/${ticket._id}/status`, { status: nextStatus }); setNotice({ type: "success", text: `Ticket moved to ${nextStatus}.` }); toast.success(`Status updated to ${nextStatus}.`, { id: `status-${ticket._id}-${nextStatus}` }); await state.load(); } catch (requestError) { const text = apiErrorMessage(requestError); setNotice({ type: "error", text }); toast.error(text, { id: `status-error-${ticket._id}` }); } finally { setStatusLoading(false); } };
  const resolve = async (event: FormEvent) => { event.preventDefault(); if (resolutionNote.trim().length < 3) return setNotice({ type: "error", text: "Resolution note must contain at least 3 characters." }); setStatusLoading(true); setNotice(null); try { await api.post(`/api/agent/tickets/${ticket._id}/resolve`, { resolutionNote: resolutionNote.trim() }); setResolveOpen(false); setNotice({ type: "success", text: "Ticket resolved successfully." }); toast.success("Ticket resolved successfully.", { id: `resolved-${ticket._id}` }); celebrateSuccess(); await state.load(); } catch (requestError) { const text = apiErrorMessage(requestError); setNotice({ type: "error", text }); toast.error(text, { id: `resolve-error-${ticket._id}` }); } finally { setStatusLoading(false); } };
